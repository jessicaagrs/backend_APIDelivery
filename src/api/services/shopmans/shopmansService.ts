...
class ShopmansService {
    ...
    async getShopmansByPagination(take: number | undefined, page: number | undefined, storeId: string | undefined) {
        if (take === undefined || page === undefined || storeId === undefined) {
            throw new Error(
                "Para buscar os vendedores, o número de registros, id da loja e a página devem ser informados."
            );
        }

        const shopmans = await repository.getShopmansByPagination(take, page, storeId);
        return shopmans;
    }

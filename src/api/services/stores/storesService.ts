...
class StoresService {
    ...
    async createStore(cnpj: string | undefined, corporateReason: string | undefined) {
        if (cnpj == undefined || corporateReason == undefined) throw new Error("O cnpj e a razão social da loja são obrigatórios");

        const cnpjExist = await repository.getStoreByCNPJ(cnpj);

        if(cnpjExist) throw new Error("CNPJ informado já existe.");

        await repository.createStore(cnpj, corporateReason);
    }

    async updateStore(id: string | undefined, cnpj: string | undefined, corporateReason: string | undefined) {
        if (id == undefined || cnpj == undefined || corporateReason == undefined) throw new Error("O id, cnpj e a razão social da loja são obrigatórios");

        const storeExist = await repository.getStoreById(id);
        
        if (!storeExist) throw new Error("Loja não encontrada");

        await repository.updateStore(id, cnpj, corporateReason);
    }

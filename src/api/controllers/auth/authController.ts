...
class AuthController {
    ...
    async createResetPassword(
        app: FastifyInstance,
        request: FastifyRequest<{ Params: Partial<ParamsType> }>,
        reply: FastifyReply
    ) {
        try {
            const { email, typeLogin } = paramsSchema.partial().parse(request.body);
            const password = await service.createResetPassword(email, typeLogin);
            return reply.status(201).send({ password });
        } catch (error: any) {
            const statusCode = reply.statusCode || 500;
            const err = new ApiError(statusCode, error.message);
            reply.status(err.statusCode).send(err);
